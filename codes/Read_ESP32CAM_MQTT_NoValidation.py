# Python program to read analaog gauge from camera ESP 32CAM
...
#Change this parameter based on the gauge
...
max_value = 100 #input('Max value: ') #maximum reading of the gauge
units = "psi" #input('Enter units: ')

#Change this value based on ESP32 Cam IP address
url = r'http://192.168.1.17/capture'
val = 0
val_before = 0
...
while True:
	try:
		...
		if (val>0):
			...
			client.username_pw_set("ESP32", "123")

			# connect to MQTT broker, set the broker and port
			client.connect("103.221.131.110", 1883)
			client.publish("gauge1", value_sent)
			client.loop_start()
		time.sleep(1.5)
